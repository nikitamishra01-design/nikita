...
function PremiumProducts() {
  ...
  // Default active tab (agar route state se aya ho)
  ...
  // Check agar koi user login me hai to
  useEffect(() => {
    const user = localStorage.getItem("user");
    if (user) setIsLoggedIn(true);
  }, []);

  // ✅ Signup handler (naya user add hoga users array me)
  ...
  // ✅ Login handler (fullName + email + password match karega)
  const handleLogin = (e) => {
    e.preventDefault();
    const fullName = e.target[0].value;
    const email = e.target[1].value;
